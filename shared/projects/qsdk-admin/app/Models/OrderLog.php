...
class OrderLog extends BusinessConnectionModel
{
    const IS_TEST_RECORD = 1;
    const IS_NOT_TEST_RECORD = 0;
    ...
    const PAYED_STATUS = 1;
    const CLOSED_STATUS = 2;

    public static function transferStatusDefinition(int $status)
    {
        switch ($status) {
            case static::NOT_PAY_STATUS:
                return '未支付';
            case static::PAYED_STATUS:
                return '已支付';
            case static::CLOSED_STATUS:
                return '已关闭';
        }
    }

    public static function transferIsTestDefinition(int $isTest): string
    {
        switch ($isTest) {
            case static::IS_TEST_RECORD:
                return '是';
            case static::IS_NOT_TEST_RECORD:
                return '否';
        }
    }
    ...
    {
        return $this->belongsTo(PostChannel::class, 'posted_channel_id', 'id');
    }
